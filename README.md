...
1. 장바구니 페이지에서 상품의 수량을 늘리거나 줄이면 값이 0원이 되는 문제 발생
    ...
    ⇒ **해결방법**: 컴포넌트 return() 안에서 계산식 직접 작성해서 cartItems 상태가 아직 초기화되지 않았고, 아직 렌더링이 되지 않아 빈 배열이었기 때문에 0이 반환되어 나온 것이다. 계산 로직을 컴포넌트 밖의 함수로 분리하고, return 안에서는 함수 호출만 하도록 구조를 바꾸어 해결하였다.
